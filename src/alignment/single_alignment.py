...
class SingleAlignment:
    ...
    def __init__(self, alignment_location):
        ...
        # Global or Local
        self.alignment_location = AlignmentAlgorithmFactory.assign_alignment_algorithm(
            alignment_location)
    ...
    def handle_input(self):
        ...
        with s1_input:
            self._set_first_sequence(st.text_area(
                label="S1: ", value="", height=self.max_height_pixels, max_chars=self.max_char_input))

        with s2_input:
            self._set_second_sequence(st.text_area(
                label="S2: ", value="", height=self.max_height_pixels, max_chars=self.max_char_input))
    ...
    def execute_alignment(self, input_txt: list, scoring_matrix, animation_speed) -> str:
        ...
        alignments, lcs, final_score = self.alignment_location.execute_alignment(
            x_axis, y_axis, scoring_matrix, animation_speed)
