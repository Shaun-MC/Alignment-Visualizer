...
from input.input_format_factory import InputFormatFactory
    ...
    def handle_input(self):
        ...
        def retrieve_sequences_input() -> str:
            ...
            uploaded_file = st.file_uploader(
                "Upload a Text File", type=["txt"])
            unclean_sequences = file_input = None

            if uploaded_file is not None:

                file_input = StringIO(
                    uploaded_file.getvalue().decode("utf-8")).read()
                ...
                if len(file_input) > self.max_char_input:
                    ...
                    text = st.text_area(
                        label="Text Input", value="", height=self.max_height_pixels, max_chars=self.max_char_input)
            ...
            unclean_sequences = (
                st.text_area(label="Text Input", value=file_input,
                             height=self.max_height_pixels, max_chars=self.max_char_input)
